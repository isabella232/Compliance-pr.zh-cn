...
Microsoft 365是高度安全的环境，提供多层的广泛保护：物理数据中心安全、网络安全、访问安全、应用程序安全和数据安全。
...
SharePoint Online 中所有客户文件都受唯一、每文件密钥的保护，这些密钥始终专用于单个租户。 密钥由 SharePoint Online 服务创建和管理，或在客户使用、创建和管理客户密钥时创建和管理。 上传文件时，在将文件SharePoint Azure 存储之前，在上传请求上下文中通过 SharePoint Online 执行加密。 下载文件时，SharePoint Online 根据唯一文档标识符从 Azure 存储检索加密的客户数据，并在将客户数据发送到用户之前对其进行解密。 Azure 存储无法解密，甚至无法识别或了解客户数据。 所有加密和解密都发生在强制执行租户隔离的相同系统中，这些隔离Azure Active Directory SharePoint Online。

Microsoft 365中的多个工作负载在 SharePoint Online 中存储数据，包括 Microsoft Teams（将所有文件存储在 SharePoint Online 中）和 OneDrive for Business（使用 SharePoint Online 进行存储）。 SharePoint Online 中存储的所有客户数据 (一个或多个 AES 256 位密钥进行加密) 并按如下方式分布在数据中心。  (此加密过程的每一步都经过 FIPS 140-2 级别 2 验证。 有关 FIPS 140-2 合规性的其他信息，请参阅 [FIPS 140-2 合规性](/previous-versions/sql/sql-server-2008-r2/bb326611(v=sql.105)).) 
...
用于访问 Azure 存储容器的凭据也保存在 SharePoint Online 密码存储中，并根据需要SharePoint每个 SharePoint Online 场。 这些凭据是 Azure 存储 SAS 签名，具有用于读取或写入数据的单独凭据，并且应用了策略，以便它们每 60 天自动过期一次。 不同的凭据用于读取或写入数据 (，) 和 SharePoint没有枚举权限。
...
有关 SharePoint Online 和 OneDrive for Business 中数据加密OneDrive for Business和 SharePoint [Online。](https://technet.microsoft.com/library/dn905447.aspx)
...
Exchange Online将 BitLocker 用于所有邮箱数据，BitLocker 配置在[BitLocker for Encryption 中进行了介绍](/microsoft-365/compliance/office-365-bitlocker-and-distributed-key-manager-for-encryption)。 服务级别加密对邮箱级别的所有邮箱数据进行加密。 

除了服务加密之外，Microsoft 365还支持基于服务加密构建的客户密钥。 客户密钥是 Microsoft 管理的用于Exchange Online加密的密钥选项，也位于 Microsoft 路线图中。 这种加密方法可提供 BitLocker 未提供的增强保护，因为它可以分离服务器管理员和解密数据所需的加密密钥，并且因为加密直接应用于数据 (与 BitLocker 相反，BitLocker 在逻辑磁盘卷上应用加密) 从 Exchange 服务器复制的任何客户数据仍保持加密状态。
...
Teams FIPS (美国联邦信息处理标准) 加密密钥交换的兼容算法。 有关 FIPS 实现详细信息，请参阅联邦信息处理标准 (FIPS) [140-2。](/microsoft-365/compliance/offering-fips-140-2)
