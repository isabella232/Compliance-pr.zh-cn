---
title: 加密Skype、OneDrive、SharePoint和Exchange
...
SharePoint Online 中所有客户文件都受唯一、每文件密钥的保护，这些密钥始终专用于单个租户。 密钥由 SharePoint Online 服务创建和管理，或在客户使用、创建和管理客户密钥时创建和管理。 上传文件时，在将文件SharePoint Azure 存储之前，在上传请求的上下文中通过 SharePoint Online 执行加密。 下载文件时，SharePoint Online 根据唯一文档标识符从 Azure 存储检索加密的客户数据，并在将客户数据发送到用户之前对其进行解密。 Azure 存储无法解密，甚至无法识别或了解客户数据。 所有加密和解密都发生在强制租户隔离的相同系统中，这些隔离Azure Active Directory SharePoint Online。

Microsoft 365中的多个工作负载在 SharePoint Online 中存储数据，包括 Microsoft Teams（将所有文件存储在 SharePoint Online 中）和 OneDrive for Business（使用 SharePoint Online 存储）。 SharePoint Online 中存储的所有客户数据 (一个或多个 AES 256 位密钥进行加密) 并按如下方式分布在数据中心。  (此加密过程的每一步都经过 FIPS 140-2 级别 2 验证。 有关 FIPS 140-2 合规性的其他信息，请参阅 [FIPS 140-2 合规性](/previous-versions/sql/sql-server-2008-r2/bb326611(v=sql.105)).) 
...
- 这些客户数据块的加密密钥集本身已加密。
    ...
    - 内容数据库受数据库访问控制和静态加密保护。 加密使用 TDE[透明数据加密 (](/sql/relational-databases/security/encryption/transparent-data-encryption-tde)中) TDE [Azure SQL 数据库。](/azure/sql-database/sql-database-technical-overview)  (Azure SQL 数据库 是 Microsoft Azure 中一种通用的 关系数据库 服务，支持关系数据、JSON、空间和 XML 等结构。) 这些密码位于 SharePoint Online 的服务级别，而不是租户级别。 这些 (有时称为主密钥) 存储在称为密钥存储的单独安全存储库中。 TDE 为活动数据库、数据库备份和事务日志提供了处于非活动状态的安全性。
...
用于访问 Azure 存储容器的凭据也保存在 SharePoint Online 密码存储中，并根据需要SharePoint每个 SharePoint Online 场。 这些凭据是 Azure 存储 SAS 签名，具有用于读取或写入数据的单独凭据，并且应用了策略，以便它们每 60 天自动过期一次。 不同的凭据用于读取或写入数据 (，) 和 SharePoint Online 场都未授予枚举权限。

> [!NOTE]
> 例如Office 365美国政府客户，数据 blob 存储在 Azure 美国政府存储。 此外，SharePoint美国政府中的 Office 365 Online 密钥的访问权限仅限于Office 365经过专门筛选的员工。 Azure 美国政府运营人员无法访问用于加密SharePoint Blob 的 SharePoint Online 密钥存储。

有关 SharePoint Online 和 OneDrive for Business 中数据加密OneDrive for Business和 SharePoint Online 中[的数据加密](https://technet.microsoft.com/library/dn905447.aspx)。

### <a name="list-items-in-sharepoint-online"></a>列出 SharePoint Online 中的项目

列表项是临时创建的客户数据的较小区块，或可在网站中更动态地工作的客户数据区块，例如用户创建的列表中的行、SharePoint Online 博客中的单个文章或 SharePoint Online Wiki 网页中的条目。 列表项存储在内容数据库中， (Azure SQL 数据库) TDE 进行保护。
...
除了服务加密之外，Microsoft 365还支持基于服务加密构建的客户密钥。 客户密钥是 Microsoft 管理的用于Exchange Online加密的密钥选项，也位于 Microsoft 路线图中。 这种加密方法可提供 BitLocker 未提供的增强保护，因为它可以分离服务器管理员和解密数据所需的加密密钥，并且加密直接应用于数据 (与 BitLocker 相反，BitLocker 在逻辑磁盘卷上应用加密) 从 Exchange 服务器复制的任何客户数据仍保持加密状态。

服务加密Exchange Online范围是静态存储在服务Exchange Online。  (Skype for Business用户邮箱中存储几乎所有用户生成的内容Exchange Online因此继承 Exchange Online.) 
