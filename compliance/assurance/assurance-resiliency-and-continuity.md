...
ms.locfileid: "49671177"
---
# <a name="resiliency-and-continuity-overview"></a>复原和连续性概述
...
BCM 管理方法的业务连续性管理 (BCM) 是 BCM 生命周期。 此三阶段过程旨在适应各种 Microsoft 业务模型实现。 它 **从评估阶段** 开始，以确定业务连续性计划应包含的关键流程和目标。 评估阶段还需要使用 BIA (业务) 。 规划 **阶段** 侧重于制定和实施复原和恢复策略，以及将其记录在正式业务连续性计划中。 最后 **，功能验证** 测试业务连续性计划及其实现，以验证有效性并确定潜在的改进。
...
为了针对各种潜在事件验证恢复和恢复策略，EBCM 计划定义了影响人员、位置和技术的多个类别的测试方案。 每个服务所需的验证级别基于服务的关键程度，而较关键服务则接受更严格的验证。 每个 Microsoft 365 服务团队都根据 EBCM 指南测试其业务连续性计划，以衡量计划的有效性和服务团队执行该计划的准备情况。
